   ...
   BinaryTree:: size( const BinaryNode * subtree )
{
	...
	{ 
		return size(subtree->left_) + size(subtree->right_) + 1;
	}
   ...
   BinaryTree:: height( const BinaryNode * subtree )
{
	...
	{
		int leftheight = height(subtree->left_);
		int rightheight = height(subtree->right_);
...
{
	while (subtree != NULL && subtree->left_ != NULL)
	{
